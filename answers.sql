...
SELECT orders.id, status, order_total FROM orders LEFT JOIN customers ON orders.customer_id = customers.id WHERE customers.email = 'pclark74@gmail.com';
...
SELECT common_name, melon_type, quantity, unit_price, total_price FROM order_items LEFT JOIN melons ON order_items.melon_id = melons.id WHERE order_id = 2725;
